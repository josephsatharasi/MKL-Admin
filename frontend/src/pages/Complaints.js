...
const Complaints = () => {
  ...
  return (
    <div>
      ...
      <Table
        columns={[
          { header: 'Customer Name', field: 'customerName', bold: true, color: '#1e3a8a' },
          { header: 'Subject', render: (complaint) => complaint.subject.length > 15 ? complaint.subject.substring(0, 15) + '....' : complaint.subject },
          { header: 'Details', hideOnMobile: true, render: (complaint) => complaint.body.length > 15 ? complaint.body.substring(0, 15) + '....' : complaint.body },
          { header: 'Date', hideOnTablet: true, render: (complaint) => new Date(complaint.createdAt).toLocaleDateString() },
          { header: 'Status', stopPropagation: true, render: (complaint) => (
            <select
          ...
          { header: 'Actions', stopPropagation: true, render: (complaint) => (
            <button 
